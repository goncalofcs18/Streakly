...
from models.schemas import HabitCreate, HabitUpdate, HabitOut
...
async def get_habits():
    db = get_db()
    habits = await db.habits.find().sort("created_at", -1).to_list(100)
...
async def delete_habit(habit_id: str):
    ...
    # cascade delete logs
    await db.logs.delete_many({"habit_id": habit_id})
